...
.Description
   ...
   Inspiration:
    ...
    Start a log file with:
    Start-CMTraceLog -Path $Global:CMLogFilePath

    Write to the log like so:
    Write-CMTraceLog -Message "Info" -Type "Info" -Component "Test"
    Write-CMTraceLog -Message "Warning" -Type "Warning" -Component "Test"
    Write-CMTraceLog -Message "Error" -Type "Error" -Component "Test"
#>

$Global:CMLogFilePath   = "C:\ContosoTemp\testplace\testcmtrace.log"
$Global:CMLogFileSize   = "40" # Log Size in KB before rolling over
...
function Write-CMTraceLog
{
    ...
    # If the log is bigger than the threshold
    if ((Get-Item $Global:CMLogFilePath).Length / 1KB -gt $Global:CMLogFileSize)
    {
        $log = $Global:CMLogFilePath

        
        $OldLog = $log.Replace(".log", ".lo_")

        # Delete previous whateverlog.lo_
        if (Test-Path $OldLog)
        {
            Remove-Item ($OldLog)
        }
